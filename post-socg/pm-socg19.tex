...
Let $f$ be a $\theta$-optimal pseudoflow in $G$ and let $f'$ be an
...
cost-scaling (i.e. we seek a $\theta^*$-optimal circulation).
...
\item For scales after the first, \textsc{Scale-Init} takes the previous
	...
	On the first scale ($\theta = \theta_0$), it does nothing.
\item \textsc{Refine} then reduces the excess to zero while maintaining
	$\theta$-optimality, turning $f$ into a $\theta$-optimal circulation.
\end{itemize}
...
Due to space constraint we have moved the proof to Appendix~\ref{SSA:num-iterations-refine}.
...
This is also the place where our assumptions on the initial cost scale $\theta_0$
become important (indeed, the only place).
We require that $\theta_0$ is sufficiently high such that $N_H$ there exists a
circulation whose bottleneck value is at most $\theta_0/2$.

\begin{lemmarep}
\label{lemma:goldberg_refine_iterations}
Suppose that $\theta_0$ is chosen such that there exists a circulation in $N_H$
with bottleneck value at most $\theta_0/2$.
...
For the first cost scale, let $\pi_0 = 0$ and $f_0$ be any circulation with bottleneck value $\leq \theta_0/2$.

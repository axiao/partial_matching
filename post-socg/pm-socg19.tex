...
$N_f$. \note{Unclear.  Doese $c_\pi(\arc wv)$ have positive or negative value for a backward arc $\arc wv$?}
...
Set $\EMPH{\underline{\theta}} \coloneqq \frac{\eps}{6k} \cdot c(e_{i^*})$.
...
We find the minimum reduced cost path of length $1$, $2$, and $3$,
...
and each Augmentation \note{\textsc?} explores $O(k)$ nodes before finding a blocking flow.
\end{lemma}

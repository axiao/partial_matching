...
\begin{enumerate}[(i)]
%\item $c(e_i) \leq \cost(M^*) \leq kn^q \cdot c(e_i)$. %TODO second half not needed?
...
%so $\cost(M) \leq kn^q \cdot c(e_i)$.
%Optimality of $M^*$ then gives $\cost(M^*) \leq kn^q \cdot c(e_i)$.
\end{proof}
...
%TODO fix description for bottleneck
\begin{lemma}

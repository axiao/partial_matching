...
Given a pseudoflow $f$, the \EMPH{imbalance} of a node (with respect to $f$) is
\[
...
Note that property~(\ref{item:scale_inv}) is satisfied in the beginning of each round.
...
the size of any cut separating the excess and deficit nodes. \note{Citation}
...
is 1, and any path passing through an dead node has a subpath of the form
...
We extend the notions of reduced cost and admissibility to alive paths,
where the reduced cost of a path is the sum of reduced costs of its edges.
Since reduced costs telescope, the reduced cost of an alive path
depends only on the potential at its (alive) endpoints.
...
Alive paths may have length 1 (no dead interior nodes), 2, or 3.
At each step, we find a minimum-reduced-cost alive path $\Pi$ from an
alive node of $X$ to an alive node of $V \setminus X$, and add the nodes
of $\Pi$ into $X$ in a single step.
We update potential in $X$ according to the reduced cost of the path.
There are $O(k)$ alive nodes, so the number of minimization queries per
Hungarian search is $O(k)$.

We find the minimum-reduced-cost alive path of length $1$, $2$, and $3$,
...
Similar to length-1 paths, we only query paths originating from $s$ if $s \in X$
...
By Lemma~\ref{lemma:cost_scale_count} below, there are $O(k)$ such changes.
...
The BCP data structure have no dependency on $X$---the only update required comes from
...
The Hungarian search periodically raises the potential for all nodes in $X$,
and we need to an efficient implementation for the data structures above.
Potential for alive nodes can be updated in a batched fashion using the
method in Section~\ref{section:hung}.

For dead nodes, notice that our data structures do not utilize their potential.
We therefore ignore them entirely and instead recover
``valid'' potential for them once they switch from dead to alive
(and additionally, at the end of each round).
Specifically, for a newly alive $a \in A$ we set $c_\pi(a) \gets c_\pi(s)$
and for newly alive $b \in B$ we set $c_\pi(b) \gets c_\pi(t)$.
% Formally, we say an alive path is \EMPH{strongly admissible}
% under $\pi$ if all of its arcs are admissible under $\pi$.
For correctness, we need to show that our choice of recovered potential
(1) preserves $\theta$-optimality and (2) makes any admissible alive
path from before augmentation admissible. \note{unparsible}
...
Both Hungarian search and Augmentation explore $O(k)$ nodes, and the blocking flow found by Augmentation is incident to $O(k)$ nodes.
% \begin{enumerate}[(i)]
% \item \label{item:cost_scale_relaxations}
% 	Both the Hungarian search and the Augmentation steps explore $O(k)$ nodes.
% \item \label{item:blocking_incidences}
% 	The blocking flow found by augmentation is incident to $O(k)$ nodes.
% \end{enumerate}
\end{lemma}

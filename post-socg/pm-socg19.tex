...
%
Move the points of each component in \emph{translation} so that the distance between points across components is greater than $kn^q \cdot c(e_i)$, but at most $O(n^q \cdot kn^{2q} \cdot c(e_i))$.
...
%TODO version diameter vs bottleneck
It remains to show that this reduction can be performed in $O(n\polylog n)$ time.
Sharathkumar and Agarwal~\cite{SA12} have shown that the partition of $A$ and
$B$ into $A_K$s and $B_K$s can be computed in $O(n \polylog n)$ time, assuming
that the indices $i$ and $j$ can be determined in such time as well.
Since $i$ is defined using the optimal partial matching, it seems difficult to
determine even given the sorted sequence $e_1, \ldots, e_{r+n-1}$.
However, we can find a polynomial approximation to $i$, which suffices for
...
%TODO probably restructure somewhat here




%TODO introduce intra/inter earlier
...
For example, the index $i$ defined above is the smallest index such that $M^*$
is an intra-cluster partial matching with respect to $T_i$.
...
%TODO i vs i': i keeps OPT intra-cluster. this is important if we distort inter-space
% but if we use j defined on i', it works right? basically, we can probably replace i with i'
\begin{lemma}
With $i'$ as defined above, $c(e_{i'}) \leq \cost(M^*) \leq kn^q \cdot c(e_{i'})$.
\end{lemma}
\begin{proof}
From definition, $i' \leq i$ and $c(e_{i'}) \leq c(e_i)$.
If $i' \neq i$, then there is at least one edge $e \in M^*$ whose endpoints
belong to two different components of $T_{i'}$, with $c(e) \geq c(e_{i'})$.
Thus, $\cost(M^*) \geq c(e_{i'})$.

Second, let $M$ be the intra-cluster $k$-matching purported to exist in $T_{i'}$.
For every edge $(a, b) \in M$, $\norm{a-b} \leq \norm{e_{i'}}$.
Each component of $T_{i'}$ has first-power diameter at most $n\norm{e_{i'}}$,
so $\cost(M) \leq kn^q \cdot c(e_{i'})$.
Optimality of $M^*$ then gives $\cost(M^*) \leq kn^q \cdot c(e_{i'})$.
\end{proof}

\begin{corollary}
Let $j$ be the first index where $c(e_j) \geq kn^q \cdot c(e_i)$.
Then, $M^*$ is an intra-cluster matching in $T_j$.
\end{corollary}
...
%TODO bottleneck version, into a lemma? into a proof?
Let $M$ be an intra-cluster $k$-matching in $T_{i'}$, which exists by definition of $i'$.
No edge in $M$ has cost more than $n^q \cdot c(e_{i'})$.
By Lemma~%TODO missing the bottleneck lemma; other section
, the cost scaling algorithm will execute correctly when initialized with
scale $C = n^q \cdot c(e_{i'})$.
Thus, we can meet the requirements of Lemma~\ref{lemma:cost_scale_approx}
choosing constant $\alpha = c(e_{i'})$.
The number of cost scales is $O(\log((n^q\alpha)/(\eps\alpha/6k)) = O(\log(n/\eps))$ for constant $q$.
...
\begin{lemma}
\label{lemma:cost_scale_approx} %TODO
Computing $(1+\eps)$-approximate geometric partial matching reduces to the following problem in $O(n \polylog n)$ time:
Given reduction network $N$ over a point set with diameter $O(\alpha \cdot kn^{2q})$
%\note{really? $K \cdot kn^{q+2}?$}
for some constant $\alpha$, compute an $(\alpha \cdot \eps/6k)$-optimal circulation on~$N$.
\end{lemma}
...
%TODO bottleneck replace? regardless, second sentence and usage of kC are erroneous
...
$O(\alpha \cdot kn^{3q})$ for some fixed value $\alpha$.

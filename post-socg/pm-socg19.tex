...
Formally, the
\EMPH{potentials $\pi(v)$} are the variables of the linear program dual to the standard linear program for the min-cost flow problem with variables $f(v,w)$ for each directed edge in $E$.
...
The proofs of the following lemmas have been moved to the appendix.
...
Our main algorithm for the unit-capacity min-cost flow problem is based on the \EMPH{cost-scaling} technique,
...
\note{this section about $\eps_0$ feels a disorganized} %TODO
...
This is insufficient for our bounds.
Instead, we use
\[
...
scale $\eps_0 = 2n^q\alpha$ that is at least the min-bottleneck value, for some fixed value $\alpha$.
For $\eps^* = \eps\alpha/6k$, as requested by Lemma~\ref{lemma:cost_scale_approx},
the number of cost scales is $O(\log(n/\eps))$.

\note{notation collision of $\eps$ for cost scale and approx quality} %TODO

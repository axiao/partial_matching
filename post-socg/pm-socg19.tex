...
The main challenge here is the set of \emph{dead vertices}
...
\emph{shortcut graph}, circumventing all dead vertices.
...
alive/dead updates caused by an augmentation, so the time for these updates
...
\end{lemma}
\begin{proof}
To sketch the proof, both procedures perform a search over the set of alive
...
alive-alive paths.
Each alive-alive path used is the result of the depth-first search in
augmentation exploring a non-explored alive node.
Applying Lemma~\ref{lemma:cost_scale_relaxations}, the total number of nodes
crossed by the flow is $O(k)$, since alive-alive paths have length at most 3.
\end{proof}
...
algorithm for computing an optimal transport map between $A$ and $B$, thereby
proving Theorem~\ref{theorem:orlin}.
...
We use a standard Hungarian search to find an augmenting path.
Let $X$ be the set of nodes it has explored, which is initially the set of
active excess nodes.
Until the algorithm reaches a deficit node, it finds a minimum reduced-cost arc
${\arc vw} \in X \times (V \setminus X)$, then uniformly raises potentials
in $X$ such that $\arc vw$ becomes admissible, and lastly adds $v$ to $X$.

The number of active excess/deficit nodes at any scale is $O(r)$, and the
...
sufficient.
We do manage to implement a Hungarian search in time roughly $r\sqrt{n}$,
...
%TODO
\newpage
\note{old transportation section}

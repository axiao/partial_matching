...
The transportation problem can also be formulated as an instance of the minimum-cost flow
problem.
...
augmenting path, we show that after an $O(n\polylog n)$ time preprocessing,
...
\etal~\cite{AFPVX17} for $r = o(\sqrt{n})$.
...
A vertex $v$ is \EMPH{matched} by matching $M$ if $v$ is the endpoint of some edge in $M$;
otherwise $v$ is \EMPH{unmatched}.
...
for each vertex $v$, called the \EMPH{potential $\pi(v)$} of $v$.
Given potentials $\pi$ \note{do we really want plur.?}, we can define the \EMPH{reduced cost} of the edges to be
$\EMPH{$c_\pi(v, w)$} \coloneqq c(v, w) - \pi(v) + \pi(w)$.
Potentials $\pi$ are \EMPH{feasible} on edge $(u,v)$ if $c_\pi(v, w)$ is nonnegative.
Potentials $\pi$ are \EMPH{feasible} if $\pi$ are feasible on every edge in $G$.
...
The Hungarian search grows a set of \EMPH{reachable vertices $X$} from
...
Add $b$ into $X$, modify potentials to make $(a, b)$ admissible,
...
\EMPH{bichromatic closest pair} (BCP) data structure, similar to \cite{AFPVX17,Vaidya89}.

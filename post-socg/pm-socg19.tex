...
the flow on $\arc vw$.}
Additionally, if there are no active excess nodes and $f(\arc vw) = 0$ for all arcs, $\Delta$
...
\begin{lemma}
$\alive{B} \setminus B^\star$ has size $O(r)$.
\end{lemma}
...
Hence, as soon as one node of $B_a^\star$ is reached, all other nodes of $B_a^\star$ are

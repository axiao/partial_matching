...
For lack of space, the proofs for lemmas in this section have been moved to the appendix.
...
The algorithms we use for solving the min-cost flow problem are primal-dual algorithms
...
To our knowledge this is folklore; we provide a proof in the appendix.
...
Here, we reduce the min-cost partial matching problem to the problem of finding 
an $\eps$-optimal circulation in a unit-capacity min-cost flow instance.
We first give an additive approximation reduction based on $\eps$-optimality,
...
We call the resulting network the \EMPH{reduction network $N_H$}.
%
...
First we show that the number of arcs used by any integer pseudoflow in $N_H$ is bounded by three times excess of the pseudoflow.
...
%TODO diameter version --- translate the components into a line separated by c(e_j)
%TODO how do we want to distinguish ||a-b|| vs the powered version? i.e., which does ``diameter'' refer to?
%\paragraph{Translated version.} %TODO remove this label

%Let $j$ be the first index where $c(e_j) \geq kn^q \cdot c(e_i)$.
%From Lemma~\ref{lemma:mult_cluster}, we have the following corollary.
%
%\begin{corollary}
%$M^*$ is an intra-cluster matching in $T_j$.
%\end{corollary}


%\begin{lemma}
%The diameter of $G_K$ is at most $2^q kn^{2q} \cdot c(e_{i'})$.
%\end{lemma}
%\begin{proof}
%The first-power diameter per component of $T_j$ is at most $(kn^q)^{1/q} \norm{e_{i'}}$.
%Each one is spaced from the next in sequence by $(kn^q)^{1/q} \norm{e_{i'}}$,
%so the first-power diameter of $G_K$ is at most $2n(kn^q)^{1/q} \norm{e_{i'}}$.
%Taking the $q$-th power, we see that the diameter of $G_K$ is at most
%$2^q kn^{2q} \cdot c(e_{i'})$.
%\end{proof}
%
%These two lemmas are sufficient for the requirements of Lemma~\ref{lemma:cost_scale_approx}
%choosing constant $\alpha = c(e_{i'})$.
%%TODO problem here
%\note{How are we guaranteed that the approx solution is intracluster? important if we translate.}
...
For a fixed $\alpha$, compute an $(\alpha\eps/6k)$-optimal circulation on a
...
originally due to Goldberg and Tarjan~\cite{GT90}.
The algorithm finds $\eps$-optimal circulations for geometrically shrinking
(halving) values of $\eps$.
Each fixed value of $\eps$ is called a \EMPH{cost scale}.
...
In what follows, we use \EMPH{$\eps^*$} to denote the ``target'' scale of the 
cost-scaling (i.e. we seek an $\eps^*$-optimal circulation).
The cost-scaling algorithm initializes the flow $f$ and the potential $\pi$ to be zero.
The initial value of the scale parameter, denoted $\EMPH{$\eps_0$}$,
is set to $C$ by the original algorithm \cite{GHKT17}.
This does not work well with the multiplicative approximation reduction (Lemma~\ref{lemma:cost_scale_approx}).
Instead, we use
\[
\eps_0 \in 2 \cdot \{x \in \reals_{\geq 0} \mid \text{there exists a circulation supported on edges of cost $\leq x$}\},
\]
or in the terminology of Lemma~\ref{lemma:cost_scale_approx}, $\eps_0 = 2n^q \alpha$.
...
\begin{itemize}
\item For scales after the first, \textsc{Scale-Init} takes the previous
	circulation (now $2\eps$-optimal) and transforms it into an
	$\eps$-optimal pseudoflow with $O(k)$ excess.
	On the first scale ($\eps = \eps_0$), it does nothing.
\item \textsc{Refine} then reduces the excess to zero while maintaining
	$\eps$-optimality, turning $f$ into an $\eps$-optimal circulation.
\end{itemize}
If $\eps \leq \eps^*$, then $f$ is an $\eps^*$-optimal circulation and we are done.
Otherwise, $\eps$ is halved and the next scale begins.
The algorithm produces an $\eps^*$-optimal circulation after $O(\log(\eps_0/\eps^*))$ scales.
Using the reduction in Lemma~\ref{lemma:cost_scale_approx}, we have an initial
scale $\eps_0 = 2n^q\alpha$ and target scale $\eps^* = \eps\alpha/6k$.
Thus, the number of cost scales after the reduction is $O(\log(n/\eps))$.

\note{We should really resolve the notation collsion between different eps: pm-approx quality, cost scale, starting scale, target scale }
...
circulation whose bottleneck value is at most $\eps_0/2$.

...
Using similar techniques, we give an exact algorithm for the planar transportation problem that runs in $O(\min\{n^2, rn^{3/2}\}\polylog n)$ time.
\end{abstract}
...
$\EMPH{$c(a, b)$} = \norm{a-b}_p^q$, for some $1 \leq p < \infty$.
...
Given $q \geq 1$, the cost of $M$, $\cost(M)$, is defined to be the sum of costs of its edges.
...
Duan~\etal~\cite{DPS18} assuming that the edge costs are integral;
...
high probability.
\note{cite the preconditioning paper?} %TODO
...
\begin{theorem}
\label{theorem:hung}
Given two point sets $A$ and $B$ in $\reals^2$ each of size at most $n$ and an
...
The main challenge here is how to deal with the set of \emph{dead nodes},
...
\cite{AFPVX17} for $r = o(\sqrt{n})$.
As in \cite{AFPVX17}, we also use the strongly polynomial uncapacitated
minimum-cost flow algorithm by Orlin~\cite{O93}, but we additional ideas are
needed to implement it faster for $r \leq \sqrt{n}$.
...
so na\"ively we can no longer spend time proportional to the size of the
support of the transportation map.
However, we ensure that the support is acyclic and use a data structure that
can find an augmenting path in $O(r\sqrt{n} \polylog n)$ time, assuming $r \leq \sqrt{n}$.
...
The Hungarian search grows a set of \EMPH{reachable vertices $X$} \emph{from}
all unmatched $v \in A$ \emph{using} augmenting paths of admissible edges.
Initially, $X$ is the set of unmatched vertices in $A$.
...
adding $b$ into $X$, and also modifying potentials to make $(a, b)$ admissible,
...
% $X$ is initialized to the set of unmatched vertices in $A$,
% and therefore $Q = B \setminus X$ has size $n$.
We observe that exactly one vertex of $A$ is newly matched after an augmentation.
...
sequence by applying the opposite insert/delete operation for each BCP update
...
The number of insertions/deletions is $O(k)$, bounded by the number of
relaxations per Hungarian search, therefore we can recover, in $O(k\polylog n)$
...
\note{I tried to condense this; please check}%TODO

\paragraph{Potential updates.}
We modify a trick from Vaidya~\cite{Vaidya89} to batch potential updates.
Potentials are tracked with a \EMPH{stored value} $\gamma(v)$, while the
...
Whenever we would raise the potentials of all vertices in $X$, we raise
...
When a vertex is removed from $X$ (e.g. by augmentation or rewinding), we update
the stored potential $\gamma(v) \gets \pi(v) + \delta$, again for the current
value of $\delta$.
Unlike \cite{Vaidya89}, we do not reset $\delta$ across Hungarian searches.
...
We say that a pseudoflow $g$ in $G_f$ is \EMPH{admissible} if $g(\arc vw) > 0$
...
Let \EMPH{$T$} be a minimum spanning tree of $A \cup B$ under the cost function $c(\cdot)$.
Let $e_1, e_2, \ldots, e_{n-1}$ be the edges of $T$ sorted in nondecreasing order
of length; in other words, $c(e_i) \leq c(e_{i+1})$.
...
Note that (\ref{item:scale_inv}) is satisfied in the beginning of each round.
...
$\supp(f)$ is incident to it; otherwise $v$ is \EMPH{alive}.
We note that $s$ and $t$ are always alive.
Let $\alive{A}$ (resp.\ $\alive{B}$, $\alive{X}$) denote the set of
alive nodes of $A$ (resp.\ $B$, $X$).
...
efficiently.
By definition, dead vertices only adjoin forward arcs in $N_f$.
Thus, the in-degree (resp.\ out-degree) of a node in $\dead{A}$ (resp.\ $\dead{B}$)
is 1, and any path passing through an dead vertex has a subpath of the form
...
We will call such paths, from an alive node to an alive node
with only dead interior nodes, \EMPH{alive-alive paths}.
We extend the notions of reduced cost and admissibility to alive-alive paths,
where the reduced cost of the path is the sum of its edges.
Since reduced costs telescope, the reduced cost of an alive-alive path
depends only on the potentials at its (alive) endpoints.

Using this observation, we implement the Hungarian search to ``skip over''
dead nodes, while logically exploring the same alive nodes in the same order.
Alive-alive paths may have length 1 (no dead interior nodes), 2, or 3.
At each step, we find a minimum reduced cost alive-alive path $\Pi$ from an
alive node of $X$ to an alive node of $V \setminus X$, and add the nodes of
this path into $X$ in a single step.
We update potentials in $X$ according to the reduced cost of the path.
There are $O(k)$ alive nodes, so the number of minimization queries per
Hungarian search is $O(k)$.

We find the minimum reduced cost alive-alive path of length $1$, $2$, and $3$,
...
This data structure finds a minimum reduced cost arc from an alive node of
...
$\alive{A} \setminus X$ (resp.\ $\alive{B} \cap X$) in a priority queues
...
closest pair (BCP) between $\alive{A} \cap X$ and $\alive{B} \setminus X$,
...
We thus maintain $\alive{A} \cap X$, $\alive{B} \setminus X$ in a dynamic
...
lose excess through augmentation, and (2) the set of dead $A$ vertices may
change; that is, when flow is augmented across a vertex of $\dead{A}$.
...
By Lemma~\ref{lemma:cost_scale_count}(\ref{item:blocking_incidences}) below, there are $O(k)$ such changes.
...
Hence, we need to find a BCP between $\dead{A}$ and $\dead{B}$,
...
The BCP sets have no dependence on $X$ --- the only updates required come from
membership changes to $\alive{A}$ or $\alive{B}$, after an augmentation.
Applying Lemma~\ref{lemma:cost_scale_count}(\ref{item:blocking_incidences}) again,
...
Note that the data structures above do not utilize potentials of dead nodes.
Potentials for alive nodes can be updated in a batched fashion using the
method in Section~\ref{section:hung}.

For dead nodes, we ignore their potentials entirely and instead recover
``valid'' potentials for them once they switch from dead to alive
(and additionally, at the end of a round).
Specifically, for a newly alive $a \in A$ we set $c_\pi(a) \gets c_\pi(s)$
and for newly-alive $b \in B$ we set $c_\pi(b) \gets c_\pi(t)$.
Formally, we will say an alive-alive path is \EMPH{strongly admissible}
...
(1) preserves $\theta$-optimality and (2) makes any admissible alive-alive
...
\begin{enumerate}
\item \label{item:cost_scale_relaxations}
	Both the Hungarian search and augmentation step explore $O(k)$ nodes.
\item \label{item:blocking_incidences}
...
$O(n\polylog n)$ preprocessing, using similar data structures.
...
Given two point sets $A$ and $B$ in $\reals^2$ of sizes $r$ and $n$ respectively
(with $r \leq \sqrt{n}$) and a supply-demand function $\tsupply: A \cup B \to \ints$
...
By applying this algorithm in the case of $r \leq \sqrt{n}$ and the one in
\cite{AFPVX17} when $r > \sqrt{n}$, we prove Theorem~\ref{theorem:orlin}.
We use a standard reduction to the uncapacitated min-csot flow problem
(see e.g. \cite{O93}) and use Orlin's algorithm~\cite{O93} as well as some of
the ideas from \cite{AFPVX17}
\note{The full version of the transportation paper is still not available anywhere.} %TODO
for implementing it faster in geometric settings.
We first present an overview of the algorithm and then describe its fast
implementation to achieve the desired running time.
...
potentials $\pi$ on the nodes.
Initially $\Delta = \fsupply(A)$, $f = 0$, and $\pi = 0$.
We fix a constant parameter $\alpha \in (0.5, 1)$.
A node $v$ is called \EMPH{active} if $\abs{\fsupply_f(v)} \geq \alpha\Delta$.
At each step, using the Hungarian search, the algorithm finds an admissible
active-excess-to-acitve-deficit path in the residual network and pushes a flow
of $\Delta$ along this path.
\footnote{Note that this augmentation may convert an excess node into a deficit node.}
It repeats this step as long as there are both active excess and active deficit nodes.
When one of these sets becomes empty, $\Delta$ is halved.
...
If $f(\arc vw) \geq 3n\Delta$, $\arc vw$ is contracted to a single node $z$ with
$\fsupply(z) = \fsupply(v) + \fsupply(w)$.
\footnote{Intuitively, $f(\arc vw)$ is so high that future scales cannot deplete
the flow on $\arc vw$.}
If there are no active excess nodes and $f(\arc vw) = 0$ for all arcs, $\Delta$
is lowered to $\max_v \fsupply(v)$.

When the algorithm terminates, it has found an optimal circulation in the
contracted network.
We use the algorithm described in \cite{AFPVX17} to recover an optimal
circulation in the original network.
Orlin showed that the algorithm terminates within $O(n\log n)$ scales and
performs a total of $O(n\log n)$ augmentations.
In the next subsection, we describe an algorithm that after $O(n\polylog n)$
preprocessing can perform a Hungarian search
(i.e.\ find an excess-deficit admissible path)
...
Here, the number of active excess/deficit nodes at any scale is $O(r)$, and the
...
The challenge to this approach is that $\supp(f)$ may have $\Omega(n)$ size, 
therefore an algorithm which runs in $O(\abs{\supp(f)})$ time is no longer
sufficient.
Still, we manage to implement a Hungarian search in time roughly $r\sqrt{n}$,
by exploiting a few properties of $\supp(f)$ as described below.

We note that each arc of $\supp(f)$ is admissible, i.e., its reduced cost is 0,
so we add an arc of $\supp(f)$ as soon as possible when it arrives in
$X \times (V \setminus X)$.
This strategy ensures the following crucial property.

\begin{lemma}
\label{lemma:supp_acyclic}
If the arcs of $\supp(f)$ are added as soon as possible, $\supp(f)$ is acyclic.
\end{lemma}

Next, similar to Section~\ref{section:goldberg}, we call a node $u \in V$
\EMPH{alive} if $u$ is an active excess/deficit node or if $u$ is incident on
an arc of $\supp(f)$.
Otherwise, $u$ is called \EMPH{dead}.
Unlike Section~\ref{section:goldberg}, once a node becomes alive it cannot
become dead.
Furthermore, a dead node may become alive only in the beginning of a scale
(after we have reduced the value of $\Delta$).
Unlike Section~\ref{section:goldberg}, an augmenting path cannot pass
through a dead node.
Therefore, we can ignore all dead nodes during Hungarian search, work with only
alive nodes, and update the set of alive nodes in the beginning of a scale.
For a subset $S \subseteq V$ of nodes, we use $\alive{S}$ to denote
the set of alive nodes in $S$.

Let $B^\star \subseteq \alive{B}$ be the set of nodes that are either active
excess/deficit nodes or that are incident on at least two arcs of $\supp(f)$.
Lemma~\ref{lemma:supp_acyclic} implies the following:

\begin{lemma}
$\abs{B^\star} = O(r)$.
\end{lemma}

We can therefore find the min reduced-cost arc $(X \cap \alive{A}) \times (B^\star \setminus X)$
using a BCP data structure as in Section~\ref{section:hung}, along with lazy
potential updates and the rewinding mechanism.
The total time spent by Hungarian search on the nodes of $B^\star$ will be $O(r\polylog n)$.
We subsequently focus on handling $\alive{B} \setminus B^\star$.

\paragraph{Handling $\alive{B} \setminus B^\star$.}
We now describe how we query a min reduced-cost arc between $(X \cap \alive{A})$
and $\alive{B} \setminus (B^\star \cup X)$.
Each node $b \in \alive{B} \setminus B^\star$ is incident on exactly one arc of
$\supp(f)$ (i.e., there is only one outgoing arc from $b$).
We partition these nodes into clusters depending on their neighbor in $N_f$.
That is, for a node $a \in \alive{A}$,
let $B_a = \{b \in \alive{B} \setminus B^\star \mid {\arc ab} \in \supp(f)\}$.
We refer to $B_a$ as the \EMPH{star} of $a$.

The crucial observation is that $a$ is the only node in $N_f$ reachable from
each $b \in B_a$, so once the Hungarian search reaches a node of $B_a$ and thus
$a$ (recall we prioritize adding $\supp(f)$ arcs), the Hungarian search need
not visit any other nodes of $B_a$, as they will only lead to $a$.
Hence, as soon as one node of $B_a$ is reached, all other nodes of $B_a$ are
discarded from further consideration.
Using this observation, we handle $\alive{B} \setminus B^\star$ as follows.

We classify each $a \in \alive{A}$ as \EMPH{light} or \EMPH{heavy}.
If $a$ is classified as heavy then $\abs{B_a} \geq \sqrt{n}$,
and if $a$ is classified as light then $\abs{B_a} \leq 2\sqrt{n}$.
Note that if $\abs{B_a} \in [\sqrt{n}, 2\sqrt{n}]$, then $a$ may be classified
as light or heavy.
We allow this flexibility to allow re-classification in a lazy manner.
Namely, a light node is re-classified heavy once $\abs{B_a} > 2\sqrt{n}$,
and a heavy node is re-classified light once $\abs{B_a} < \sqrt{n}$.
This scheme ensures that the star of $a$ has gone through at least $\sqrt{n}$
updates (insertion/deletion of nodes) between two successive re-classifications,
and these updates will pay for the time spent in updating the data structure
when $a$ is re-classified --- see below.

For each heavy node $a \in \alive{A} \setminus X$, we maintain a BCP data
structure between $B_a$ and $X \cap \alive{A}$.
Next, for all light $a \in \alive{A} \setminus X$, we collect their stars into
a single set $B^< = \bigcup_{\text{$a$ light}} B_a$.
We maintain a BCP data structure between $B^<$ and $\alive{A} \cap X$.
Thus, we maintain at most $r$ different BCP data structures for stars.

Using these data structures, we can compute a min reduced-cost arc between
$\alive{A} \cap X$ and $\alive{B} \setminus (B^\star \cup X)$.
Once an arc ${\arc vw} \in (\alive{A} \cap X) \times (\alive{B} \setminus (B^\star \cup X))$
is added such that $w \in B_a$ for some star $B_a$, then we also add $a$ to $X$.
If $a$ is light, then we delete $B_a$ from $B^<$ and update the BCP data structure for $B^<$.
If $a$ is heavy, then we stop querying the BCP data structure on $B_a$ for the
remainder of the search.
Finally, since $a$ becomes a part of $X$, $a$ is added to all $O(r)$ BCP data structures.

Recall that $r \leq \sqrt{n}$ by assumption.
Adding the arc $\arc vw$ thus involves performing $O(\sqrt{n})$
...
excess/deficit nodes and $\supp(f)$ arcs change.
We thus need to update the set $B^\star$, $B_a$, and $B^<$.
...
can be recovered from that in the contracted network in $O(n\log n)$
time~\cite{AFPVX17}, the total running time of the algorithm is

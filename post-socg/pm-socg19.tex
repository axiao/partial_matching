...
this is important to rewinding.
...
For subsequent rounds, we begin with $f$ as a $2\theta$-optimal circulation in $N$.
First, we raise the potential of all $a \in A$ by $\theta$, all $b \in B$ by $2\theta$,
and of $t$ by $3\theta$.
The potential of $s$ is unchanged.
%
The above step increases the reduced cost of all forward arcs to at least
$-2\theta + \theta = -\theta$.
...
The time required for \textsc{Scale-Init} is $O(n)$.
...
the size of any cut separating the excess and deficit nodes (Lemma~4~\cite{GHKT17}).
...
%TODO i think this might be able to be compressed
...
(1) preserves $\theta$-optimality and (2) makes any alive path used by the most
recent blocking flow admissible (per-arc, instead of in sum).
It is straightforward calculation to verify both properties.

The following lemma is crucial to analyzing the running time of the Hungarian search.
...
Agarwal \etal~\cite{AFPVX17} when $r > \sqrt{n}$, we prove Theorem~\ref{theorem:orlin}.
...
the ideas from Agarwal \etal~\cite{AFPVX17}
\note{Cite full version} %TODO
for efficient implementation under the geometric settings.
...
Initially $\Delta = \fsupply(A)$ \note{undef?}, $f = 0$, and $\pi = 0$.
...
We use the algorithm described in Agarwal~\etal~\cite{AFPVX17} to recover an optimal
...
so we relax an arc of $\supp(f)$ as soon as possible when it arrives in
$X \times (V \setminus X)$, with priority over non-support arcs.
Crucially, this gives:

\begin{lemma}
\label{lemma:supp_acyclic}
If the arcs of $\supp(f)$ are relaxed as soon as possible, $\supp(f)$ is acyclic.
\end{lemma}

Next, similar to Section~\ref{section:goldberg}, we call node $u$
\EMPH{alive} if (a) $u$ is an active excess/deficit node or (b) if $u$ is incident to
...
alive nodes, and update the set of alive nodes at the beginning of a scale.
...
excess/deficit nodes or (b) incident to \emph{exactly one} arc of $\supp(f)$.
...
$a$ (we prioritize relaxing $\supp(f)$ arcs), the Hungarian search need
...
excess/deficit nodes and the support arcs change.

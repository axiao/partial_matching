...
Abusing the notation, we use $c_\pi$ to denote the reduced cost of arcs in
$N_f$, in addition to $N$.

The \EMPH{dual feasibility constraint} asks that $c_\pi(\arc vw) \geq 0$ holds
for every arc $(v,w)$ in $\vec{E}$;
...
\note{If we want to move scale-init to a post-processing, this invariant needs more proof (potentials being updated by scale-init, thus previous circulation is no longer a certificate)} %TODO
...
\note{Keep this invariant and move scale-init to a post-processing after halving theta}%TODO
...
The end of this path is the next active node that the original Hungarian search
would have explored.
We update potentials in $X$ according to the total reduced cost of this path.
...
\note{for now: write this to reference rewinding; and ``additionally'' wrt active-switch} %TODO
...
change, i.e. when flow is augmented across a vertex of $A \setminus \overline{A}$.
...
blocking flow found in Augmentation passes through it.
...
We now describe how to find the cheapest path of the form $s \to v \to w \to t$
where $v \in A \setminus \overline{A}$ and $w \in B \setminus \overline{B}$.
Note that $c_\pi(s \to v \to w \to t) = c(\arc vw) - \pi(s) + \pi(t)$.
Hence, we need to find a BCP of $A \setminus \overline{A}$, $B \setminus \overline{B}$,
...
Specifically, for newly-active $a \in A$ we set $c_\pi(a) = c_\pi(s)$
and for newly-active $b \in B$ we set $c_\pi(b) = c_\pi(t)$.
It is straightforward to verify that this choice preserves $\theta$-optimality
throughout $f$ as well the admissibility of any length-2/length-3 paths that
involved them.

\note{do we need to extend the definition of admissibility to paths?} %TODO
...
and Augmentation explores $O(k)$ nodes before finding a blocking flow.
\end{lemma}
\begin{proof}
...
active nodes connected by a subpath across 0, 1, or 2 inactive nodes.
Each such subpath corresponds to the depth-first search of Augmentation
exploring a new active node.
By Lemma~\ref{lemma:cost_scale_relaxations}, the total number of nodes
crossed is therefore $O(k)$.
\end{proof}
...
\note{need to show $\sqrt{k}$ iterations per refine} %TODO
...
\newpage
\note{old section 3 (to remove)} %TODO

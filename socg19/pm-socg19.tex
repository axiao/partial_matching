...
bounded above by twice of its length.  There are $O(rn\log n)$ augmentations in total.
%(each vertex may be removed from one star, and/or added to another star).
Each membership change can be performed in $O(\polylog n)$ time.
...
After $O(rn\polylog n)$ preprocessing, we spend $O(r\sqrt{n}\polylog n)$ time on
each Hungarian search by Lemma~\ref{lemma:orlin_hs_time}.
After each augmentation, we spend the same amount of time
%(plus $O(\polylog n)$ extra)
to initialize data structures for the next Hungarian search.
We spend up to $O((r^2\sqrt{n} + rn)\polylog n)$ time making the switch between big/small stars.

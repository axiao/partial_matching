...
size-$k$ matching in $G(A, B)$ the \EMPH{geometric partial matching problem (MPM)}.
...
$\|a - b\| - \omega(a) + \omega(b)$ for some real-valued vertex weights $\omega$. \note{$d(a, b)$?}
...
During each relaxation, we perform at most one query and add a vertex to $S$ incurring one BCP insertion or deletion.
...
We adapt a trick from Vaidya~\cite{Vaidya89} to batch potential updates under the rewinding mechanisim,
...
Now we employ a technique from Sharathkumar and Agarwal~\cite{SA12} to convert the additive approximation into a multiplicative one, thus proving Lemma~\ref{lemma:cost_scale_approx}.  See Appendix~\ref{SSA:multiplicative-approx} for details.
...
%
Combined with the $O(\sqrt{k})$ bound on the number of iterations we proved in Section~\ref{SS:cost-scaling}, the procedure \textsc{Refine} can be implemented in $O((n+k\sqrt{k}) \polylog n)$ time.  Together with our analysis on scale initialization and the bound on number of cost scales, this concludes the proof to Theorem~\ref{theorem:gmcm}.
...
problem in $O(rn(r/\sqrt{n} + \sqrt{n}) \polylog n)$ time, proving Theorem~\ref{theorem:orlin}.
...
implemented in $O(r(r/\sqrt{n}+\sqrt{n})\polylog n)$ time (after preprocessing).
...
In this case, the set of admissible arcs under an optimal potential forms a planar graph, and thus we can apply the planar maximum-flow algorithm~\cite{E10,xxx} which runs in $O(n \log n)$ time.  For details see Appendix~\ref{SSA:flow-recovery} and~\ref{SSA:flow-recovery-planar}.

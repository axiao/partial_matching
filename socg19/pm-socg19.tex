...
We present the first set of algorithms for geometric partial matching that work for any powers of $L_p$-norm matching objective and where the running time is near-linear in $n$:
...
Given two point sets $A, B$ in $\reals^2$, we consider the problem of finding
...
For a given $q \geq 1$, the cost of $M$ is defined to be
\begin{equation*}
...
\footnote{Sometimes the partial matching problem is also called the \EMPH{imperfect matching} or \EMPH{imperfect assignment} \cite{RT12,GHKT17}.}

We also consider the \emph{transportation problem}, which is a generalization of the bipartite matching problem.
...
The transportation problem is essentially the discrete form of the
...
Both minimum-cost bipartite matching and maximum-cardinality bipartite
...
algorithm by Even and Tarjan~\cite{ET75}.
The first improvement over \cite{HK73,ET75} in over thirty years was the 
interior point algorithm by Madry~\cite{M13}, which runs in
$O(m^{10/7}\polylog n)$ time.

The Hungarian algorithm~\cite{Kuhn55} computes a minimum-weight
maximum matching in a bipartite graph in roughly $mn$ time, where $n$ is the
number of vertices and $m$ is the number of edges.
Faster algorithms have been developed since for minimum-weight bipartite
matching, such as the $O(m\sqrt{n}\log(nC))$ time algorithms by Gabow and
Tarjan~\cite{GT89} and the $O(m\sqrt{n}\log C)$ time algorithm by
Duan~\etal~\cite{DPS11}.
The Hungarian algorithm can be extended to compute a minimum-cost partial
matching of size $k$ in time $O(km + k^2\log r)$ time, see e.g. \cite{RT12}.
Ramshaw and Tarjan~\cite{RT12} proposed a cost-scaling algorithm for partial
...
developed a cost-scaling algorithm for partial matching with
$O(m\sqrt{k}\log(kC)) = O(nr\sqrt{k}\log(kC))$ running time, again only for
integral edge costs.

In geometric settings, the Hungarian algorithm can be implemented to compute
an optimal perfect matching between $A$ and $B$ (assuming $\abs{A} = \abs{B} = n$)
in time $O(n^2\polylog n)$~\cite{KMRSS17} (see also \cite{Vaidya89,AES99}).
This algorithm can compute an optimal size-$k$ matching in time $O(kn\polylog n)$.
...
The best known algorithm, by Sharathkumar and Agarwal~\cite{SA12m}, can
compute a $(1+\eps)$-approximation of the optimal perfect matching in
$O(n\eps^{-O(1)}\polylog n)$ expected time with high probability,
assuming $q=1$, i.e. the cost of a matching is the sum of edge costs and the
cost of an edge $(a, b)$ is $\norm{a-b}_p$ for some $p \geq 1$.
Their algorithm can also compute a $(1+\eps)$-approximate optimal partial
matching within the same time bound.
For $q > 1$, the best known approximation algorithm for computing a perfect
matching runs in $O(n^{3/2}\log(1/\eps)\polylog n)$ time \cite{SA12}.
It is not obvious how to extend this algorithm to compute an approximate
partial matching.
...
transformations of $B$, or (ii) to compute a set $\EuScript{M}$ of matchings
so that for any translation/rotation $\mathbb{T}$ of $B$, a (near) optimal
matching of $A$ and $\mathbb{T}(B)$ is in the set $\EuScript{M}$.
Many of these algorithms use a partial matching algorithm on static points as a
black box, so our results improve the final time bounds in e.g.
\cite{R10,AHJKRST18,AKKMRSX18}.

The transportation problem can be formulated as a \EMPH{minimum-cost flow}
problem in a graph.
...
Lee and Sidford give a weakly polynomial algorithm that runs in
$O(m\sqrt{n}\polylog(n, U))$ time.

Agarwal~\etal have shown that Orlin's algorithm can be implemented in the
geometric setting in time $O(n^2\polylog n)$.
It is an open question whether this algorithm can be implemented to run in
$O(nr\polylog n)$ time (recall that $\abs{A} = n$, $\abs{B} = r$).
By adapting the Lee-Sidford algorithm, they developed an
$(1+\eps)$-approximation algorithm with $O(n^{3/2}\eps^{-2}\polylog n)$
running time.
They also gave a Monte-Carlo algorithm that computes a
...
First (Section~\ref{section:hung}), we present an efficient algorithm for
computing an optimal partial matching in $\reals^2$.

\begin{theorem}
\label{theorem:hung}
Given two point sets $A, B$ in $\reals^2$, with $\abs{A}, \abs{B} \leq n$,
a minimum-cost matching of size $k$ between $A$ and $B$ can be computed in
$O((n + k^2)\polylog n)$ time.
\end{theorem}

As in \cite{KMRSS17,AES99}, we use \emph{bichromatic closest pair}
(BCP) data structures to implement the Hungarian algorithm efficiently.
But unlike \cite{KMRSS17,AES99}, which take $\Omega(n)$ time to find an
augmenting path, we show that after $O(n\polylog n)$ preprocessing an
an augmenting path can be found in $O(k\polylog n)$ time, without starting
a completely new \emph{Hungarian search}.
We refer to this idea as a \emph{rewinding} mechanism, and it will be used
in our other algorithms as well.

Next (Sections~\ref{section:goldberg},\ref{S:implementation}),
by adapting the unit-capacity min-cost flow algorithm by
Goldberg~\etal~\cite{GHKT17} and implementing it efficiently
we obtain a $(1+\eps)$-approximation algorithm for the 2D geometric partial
matching problem:

\begin{theorem}
\label{theorem:gmcm}
Given two point sets $A, B$ in $\reals^2$, with $\abs{A}, \abs{B} \leq n$,
a $(1+\eps)$-approximate minimum-cost matching of size $k$ between $A$
and $B$ can be computed in $O((n + k\sqrt{k})\log(1/\eps)\polylog n)$ time.
\end{theorem}

The main challenge for this algorithm is what we call \emph{null vertices},
which do not contribute to an augmentation, but for which the algorithm may
spend $\Omega(n)$ time every augmentation.
...
The shortcut graph itself may have $\Omega(n^2)$ edges, but we can query its
minimum arcs efficiently using a BCP oracle without explicit construction.
...
Finally (Section~\ref{section:orlin}) we present a faster algorithm for the
transportation problem when the two point sets are unbalanced i.e. $\abs{A} = r$,
$\abs{B} = n$ and $r \ll n$:

\begin{theorem}
\label{theorem:orlin}
Given two point sets $A, B$ in $\reals^2$ of sizes $r$ and $n$ respectively
along with supply-demand function $\tsupply:A \cup B \to \ints$, an optimal
transportation map between $A$ and $B$ can be computed in
...
Unlike matchings, the flow support in transportation may have size $\Omega(n)$
even if $r = O(1)$, so it seems difficult to charge execution time to flow support
as before.
...
$O(r)$.
We describe a data structure that can process these stars in amortized

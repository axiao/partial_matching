...
Applying combinatorial algorithms for partial matching in general graphs to our setting naively requires quadratic time due to existence of many edges between point sets $A$ and $B$.
Most previous work for geometric matching has focused on the setting when $k$, $r$, and $n$ are equal.
The best algorithm in this setting, due to Sharathkumar and Agarwal [STOC 2012], runs in time $O(n\polylog n \poly\eps^{-1})$, but is limited to matching objectives that are sum-of-distances.

We present the first set of geometric algorithms which work for any powers of $L_p$-norm matching objective:  An exact algorithm which runs in $O((n + k^2)\polylog n)$ time, and a $(1 + \eps)$-approximation which runs in $O((n + k\sqrt{k})\polylog n \log\eps^{-1})$ time.
...
%
% If an arc $(v, w)$ is newly introduced this way, then by definition of residual
...
stored potential.
Since all the points of $S \cap A$ have weights uniformly offset from their

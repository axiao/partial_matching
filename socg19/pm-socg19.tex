...
\begin{abstract}
Let $A$ and $B$ be two point sets in the plane with uneven sizes $r$ and $n$ respectively (assuming $r$ is at most $n$), and let $k$ be a parameter. The geometric partial matching problem asks to find the minimum-cost size-$k$ matching between $A$ and $B$ under powers of $L_p$ distances.
Applying combinatorial algorithms for partial matching in general graphs to our setting na\"ively requires quadratic time due to existence of many edges between point sets $A$ and $B$.
Most previous work for geometric matching has focused on the setting when $k$, $r$, and $n$ are all equal.
The best algorithm in this setting, due to Sharathkumar and Agarwal [STOC 2012], runs in time $O(n\polylog n \cdot \poly\eps^{-1})$, but is limited to matching objectives that are sum-of-distances.

We present the first set of geometric algorithms which work for any powers of $L_p$-norm matching objective:  An exact algorithm which runs in $O((n + k^2)\polylog n)$ time, and a $(1 + \eps)$-approximation which runs in $O((n + k\sqrt{k})\polylog n \cdot \log\eps^{-1})$ time.
...
Using similar techniques, we give an exact algorithm for the planar transportation problem, which runs in $O(rn^{3/2}\polylog n)$ time.
...
Given two point sets $A, B$ in the plane, we consider the problem of finding
the minimum-cost size-$k$ matching between $A$ and $B$.
Formally, suppose $\abs{A} = r$ and $\abs{B} = n$, with $r \leq n$.
Let $G(A, B)$ be the undirected complete bipartite graph between
$A$ and $B$, and let the cost of $(a, b) \in A \times B$ be
$c(a, b) = \norm{a-b}_p^q$, for some $1 \leq p < \infty$ and $q \geq 1$.
Define $C \coloneqq \max_{(a, b) \in A \times B} c(a, b)$.
A \EMPH{matching} $M$ in $G(A, B)$ is a set of edges sharing no endpoints,
and the cost of $M$ is 
\begin{equation*}
	\EMPH{$\cost(M)$} = \sum_{(a, b) \in A \times B} \norm{a-b}_p^q.
\end{equation*}
For a parameter $k \leq r$, we call the problem of finding the minimum-cost
size-$k$ matching in $G(A, B)$ the \EMPH{geometric partial matching} problem.
We call the respective problem in general bipartite graphs (with arbitrary
edge costs) the \EMPH{partial matching} problem.
This has also been called the \EMPH{imperfect matching} problem.

The second problem we consider is a vertex-weighted variant of bipartite
matching called the \emph{transportation problem}.
Let $\tsupply:A \cup B \to \ints$ be a \EMPH{supply-demand function} with
positive value on points of $A$, negative value on points of $B$, satisfying
$\sum_{a \in A} \tsupply(a) = - \sum_{b \in B} \tsupply(b)$.
Define $U \coloneqq \max_{p \in A \cup B} \abs{\tsupply(p)}$.
...
be sent to meet demands at $b$, and $\tau$ is feasible if all supplies are sent
and all demands are met.
We define the cost of $\tau$ to be
\begin{equation*}
	\EMPH{$\cost(\tau)$} \coloneqq \sum_{(a, b) \in A \times B} \norm{a-b}_p^q \cdot \tau(a, b).
\end{equation*}
The \EMPH{transportation problem} asks to find a feasible transportation map of
minimum cost.
The cost itself is called the \EMPH{optimal transport cost} or the
\EMPH{earth mover's distance}.
We focus on the setting where $r < n$, i.e. the point sets are \EMPH{unbalanced}
(although the total supply and demand are balanced).

\subsection{Related work}

Non-geometric algorithms for partial matching can be applied to $G(A, B)$,
which is a graph with $r+n$ vertices and $m \coloneqq rn$ edges.
There is a wealth of literature on general min-cost bipartite matching between 
\EMPH{balanced} vertex sets ($r = n$) finding a \EMPH{perfect matching}
($k = r = n$).
Not all algorithms for perfect matching can be easily modified to solve partial
matching, with a notable exception being the celebrated primal-dual
\EMPH{Hungarian Algorithm}~\cite{Kuhn55}.
The Hungarian Algorithm can be ``stopped after $k$ iterations'' to solve partial
matching in $O(km + k^2\log r) = O(knr + k^2\log r)$ time, see e.g. \cite{RT12}.
Ramshaw and Tarjan~\cite{RT12} give a cost-scaling algorithm for partial
matching which runs in time $O(m\sqrt{n}\log(nC)) = O(n^{3/2}r\log(nC)$,
but only when costs are integral (for distances, they generally are not).
By reduction to unit-capacity min-cost flow, Goldberg~\etal~\cite{GHKT17}
give a cost-scaling algorithm for partial matching which runs in
$O(m\sqrt{k}\log(kC)) = O(nr\sqrt{k}\log(kC))$ time, again only for integral
costs.

Similarly, many geometric perfect matching algorithms do not convert easily
into geometric partial matching algorithms.
However, the $(1+\eps)$-approximation algorithm due to Sharathkumar and
Agarwal~\cite{SA12m} can be modified to solve partial matching by ``stopping at
the $k$-th iteration,'' to solve geometric partial matching in
$O(n\poly(\log n, 1/\eps))$ time, when costs are the $p$-norm.
We note that these costs are less general than the setting in this paper
(``$q$-th power of the $p$-norm'').
For instance, Sharathkumar-Agarwal does not support squared Euclidean costs,
i.e. \EMPH{root mean squared} (RMS) matching costs.

The transportation problem can similarly be formulated as a
\EMPH{minimum-cost flow} problem on $G(A, B)$.
Thus, the strongly polynomial uncapacitated min-cost flow algorithm by
Orlin~\cite{O93} solves transportation in
$O((m + n\log n)n\log n) = O(n^3\log n + n^2\log^2 n)$ time.
Lee and Sidford give a weakly polynomial algorithm which runs in
$O(m\sqrt{n}\polylog(n, U)) = O(n^{3/2}r\polylog(n, U))$ time.

Another line of approximation algorithms for transportation use an entropy
regularizer to pose it as a \EMPH{matrix scaling problem}, following the work
of Cuturi~\cite{C13}.
The regularized problem admits a simple iterative algorithm called the
\EMPH{Sinkhorn-Knopp algorithm}.
Theoretical bounds for the Sinkhorn-Knopp algorithm have emerged only recently:
Sinkhorn-Knopp finds an additive $\eps$-approximation in time
$O(\frac{n^2}{\eps^2}\polylog n)$ due first to Altschuler~\etal~\cite{AWR17}
and improved by Dvurechensky~\etal~\cite{DGK18}, under the very general setting
of \emph{nonnegative cost matrices}.
For costs $\norm{\cdot}_2^2$ (RMS costs) Altschuler~\etal~\cite{ABRW18} are
able to modify the additive approximation to run in
$O(\frac{n}{\eps}(\log n/\eps)^d)$ time, for points in dimension $d$.

In terms of geometric specialty algorithms, Agarwal~\etal~\cite{AFPVX17}
give an expected $O(\log^2(1/\eps))$ approximation running in $O(n^{1+\eps})$
expected time, an $(1+\eps)$-approximation algorithm running in
$O(n^{3/2}\polylog(n, U))$ time, and an exact $O(n^2\polylog n)$ time algorithm.
Whether a near-linear time algorithm exists for $(1+\eps)$-approximating the
transportation problem with geometric costs is still open.

\subsection{Contributions}

We present two algorithms for geometric partial matching that are based on
fitting nearest-neighbor (NN) and geometric closest pair (BCP) oracles into
primal-dual algorithms for non-geometric bipartite matching and minimum-cost flow.
This pattern is not new, see for example \cite{Vaidya89,AV95,SA12,AFPVX17},
but allows us to push the analysis further elsewhere.
...
Our main technique is something we call a \emph{rewinding mechanism} to 
quickly initialize the (size $O(n)$) data structures in each iteration.
Roughly, the initial data structures for two consecutive iterations differ by
only one point, so we can generate the new initialization by ``undoing'' the
sequence of insertions/deletions from the last iteration and performing one
additional update operation.

\begin{theorem}
\label{theorem:hung}
A minimum-cost geometric partial matching of size $k$ can be computed between
$A$ and $B$ in $O((n + k^2)\polylog n)$ time.
\end{theorem}

% O((n + k\sqrt{k})\polylog n\log(n/\eps))

Next in Section~\ref{section:goldberg}, we apply a similar technique to the
unit-capacity min-cost flow algorithm of Goldberg, Hed, Kaplan, and
Tarjan~\cite{GHKT17}.
...
The main issue for this algorithm is what we call \emph{null vertices}, which
do not contribute to an augmentation, but for which the algorithm may waste
time examining.
Instead, we run the unit-capacity min-cost flow algorithm on a
\emph{shortcut network} which has paths circumventing all null vertices.
The shortcut graph itself may have $O(n^2)$ arcs, but we can query its minimum
arcs efficiently using a BCP oracle without explicit construction.
Once we have done this, we are able to charge the execution time of the
iteration to the size of the \emph{flow support}, the set of arcs with positive
flow, which turns out to be size $O(k)$.

\begin{theorem}
\label{theorem:gmcm}
A $(1+\eps)$ geometric partial matching of size $k$ can be computed between
$A$ and $B$ in $O((n + k\sqrt{k})\log(1/\eps)\polylog n)$ time.
\end{theorem}

% add the ~O(rn^{3/2}) version

Our third algorithm solves the transportation problem in the unbalanced setting
using the strongly polynomial uncapacitated min-cost flow algorithm by Orlin~\cite{O93},
adapated for geometric costs as in Agarwal~\etal~\cite{AFPVX17}.
The result is an $O(r\sqrt{n}(r+\sqrt{n})\polylog n)$ time exact algorithm
for unbalanced transportation.
This improves over the $O(n^2 \polylog n)$ time exact algorithm in
Agarwal~\etal~\cite{AFPVX17} when $r = o(\sqrt{n})$.
Unlike matching, the flow support may have $\omega(n)$ size even if $r = O(1)$,
so it seems like we may be unable to charge to flow support as before.
However, we show that most of these support arcs are degree one and partition
into \emph{stars} centered around vertices of $A$, and the remainder is size
$O(r)$.
Unfortunately, we are not able to handle these stars as easily in our data
structures, and each iteration is dominated by the time spent maintaining
data structures representing stars.
...
In this section, we prove Theorem~\ref{theorem:hung} by implementing the Hungarian algorithm in $O((n + k^2)\polylog n)$ time.
...
The linear program dual to the standard linear program for MPM has dual variables for
...
An \EMPH{augmenting path}
$\Pi = (a_1, b_1, \ldots, a_\ell, b_\ell)$ is an odd-length path with unmatched
endpoints ($a_1$ and $b_\ell$) and all other points matched.
The edges of $\Pi$ alternate between edges outside and inside of matching $M$.
The symmetric difference $M \oplus \Pi$ creates a new matching of size $\abs{M}+1$.
We say that $M \oplus \Pi$ is the result of \EMPH{augmenting} $M$ by $\Pi$.
...
$O((n + k^2)\polylog n)$ time, which proves Theorem~\ref{theorem:hung}.
...
%In general graphs,
The most expensive step in augmentation is to find the minimum-reduced-cost
frontier edge that needs to be relaxed --- the search must ``look at every edge''.
%e.g.\ by pushing them into a priority queue even if they are not relaxed.
In the geometric setting, we find the min-cost edge using a dynamic
...
The state of the art dynamic BCP data structure from Kaplan \etal~\cite{KMRSS17} supports point insertions and deletions in
...
Recall that in the beginning of each iteration,
$S$ is initialized to the set of unmatched vertices in $A$,
and therefore $Q = B \setminus S$ has size $n$ \note{what?}.
We cannot afford to take $O(n\polylog n)$ time initialize the BCP data structure at the
beginning of every Hungarian search beyond the first.
However, the set of unmatched $A$ vertices has changed by exactly one
...
Hungarian search, we can construct $P$ and $Q$ for the current iteration
%by taking $Q \gets Q'$ and $P \gets P' \setminus \{a^*\}$.
%In other words, we only need
using a single BCP deletion in $O(\polylog n)$ time.

To acquire $P'$ and $Q'$, we keep track of a list of the points added to $S$
over the course of the Hungarian search.
At the end of each Hungarian search we \emph{rewind} the BCP data structure by tracing the list in reverse order.
...
\note{REWRITE TO BE MORE SUCCINCT.}
...
\note{Summarize our new ideas that lead to the improvement.}
...
Due to the space restriction, we omit the definitions of standard network flow theory terminologies from the main text.  For a reference see Appendix~\ref{SSA:prelim-flow}, or any texts on network flows \cite{}.
%
...
The \EMPH{support} of a pseudoflow $f$ in $N$, denoted as \EMPH{$\supp(f)$}, is the set of arcs with positive flows:
\(
...
%
We sometimes abuse the terminology by allowing pseudoflow to be defined on a directed graph, in which case we are actually referring to the pseudoflow on the corresponding network by extending the flow values antisymmetrically to the arcs.
%
...
Next we employ a technique by Sharathkumar and Agarwal~\cite{SA12} to transform an additive $\eps$-approximate solution into a multiplicative $(1+\eps)$-approximation for the geometric partial matching problem.
The reduction does not work out of the box, as Sharathkumar and Agarwal were tackling a similar but different problem on geometric transportations.
...
\note{Is it okay if we hide the parametric search?}

Sharathkumar and Agarwal~{\cite[\S3.5]{SA12}} provide a construction that partitions the input point sets $A$ and $B$ for the MPM problem into clusters, such that the diameter of each cluster is upper bounded by the cost of the optimal partial matching multiply by a polynomial factor.
%
...
In total, potential updates and backward arc desaturations, thus the whole procedure \textsc{Scale-Init}, take $O(n)$ time.
...
%
Combined with the $O(\sqrt{k})$ bound on the number of iterations we just proved, the procedure \textsc{Refine} can be implemented in $O((n+k\sqrt{k}) \polylog n)$ time.  Together with our analysis on scale initialization and the bound on number of cost scales, this concludes the proof to Theorem~\ref{theorem:gmcm}.
...
\end{itemize}
For arcs in each category we maintain a proper data structure (either heap or BCP) to query the min-cost arc.
...
The complete time analysis can be found in Appendix~\ref{SSA:num-relaxation} and~\ref{SSA:time-ananlsis}; here we sketch the ideas.
%
...
%
As for the time analysis, using the same rewinding mechanism as in Section~\ref{SS:fast-hungarian-matching}, the running time of the Hungarian search and depth-first search, other than the potential updates, can be charged to the number of relaxations.
Again using the trick by Vaidya~\cite{Vaidya89} in Section~\ref{SS:fast-hungarian-matching} we can charge the potential updates of normal vertices to the number of relaxations in the Hungarian search.
%
...
problem in $O(rn^{3/2} \polylog n)$ time, proving
Theorem~\ref{theorem:orlin}.
Our strategy is to use the standard reduction to the uncapacitated
min-cost flow problem, and provide a fast implementation under the geometric setting for the uncapacitated min-cost flow
algorithm by Orlin~\cite{O93}, combined with some of the tools developed in Sections~\ref{section:hung} and~\ref{section:goldberg}.
...
We give an outline of the strongly polynomial algorithm for uncapacitated min-cost flow problem
from Orlin~\cite{O93}.
...
and each augmentation over admissible edges preserve such property by
...
implemented in $O(r\sqrt{n}\polylog n)$ time (after
preprocessing).
...
with real points in the plane instead of the the contracted components.
...
If furthermore the coinadmissiblee is just the $p$-norm (without the $q$th-power), an even stronger result stands:
In this case, the set of admissible arcs under an optimal potential forms a planar graph, and thus we can apply the planar maximum-flow algorithm~\cite{E10,xxx} which runs in $O(n \log n)$ time.  For details see the appendix.
...
can be amortized to the insertions/deletions that preceded the switch, plus

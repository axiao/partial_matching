...
maximal), we say the matching instance is \EMPH{unbalanced}.
...
%
\note{talk about the near-linear time algorithm}%TODO
% \begin{TODO}
% Previous work
...
to satisfy a unit of demand at $b \in B$ is $\|a - b\|$.
...
We describe a geometric implementation of the uncapacitated min-cost flow
...
Let $A$ and $B$ be point sets in the plane.
%with $r \coloneqq \abs{A}$ and $n \coloneqq \abs{B}$.
...
We define the \EMPH{cost} of $\tau$ to be
\[
...
We focus on the case when the point sets are \EMPH{unbalanced}; that is,
the given point sets have different sizes $r$ and $n$; without loss of generality assuming $r \le n$.
...
some SPT arcs may be inadmissble.
...
Recall that $f$ has values defined only on the un-contracted residual arcs;
...
\subsection{Recovering the optimal flow for $1$-power.}

When the matching objective uses the $1$-power of $p$-norms, we can prove that
the subgraph formed by admissible arcs is planar.

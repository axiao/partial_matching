...
\EventLogo{eatcs}
\SeriesVolume{}
\ArticleNo{100} % Set article-no=1
...
The number of relatexations in a Hungarian search is $O(k)$ by Corollary~\ref{corollary:goldberg_hs_length}.
...
bounded by the number of relaxations in the Hungarian search, which is $O(k)$ by Corollary~\ref{corollary:goldberg_hs_length}.
...
Up until now, we have not placed restrictions on coincidence between $A$ and
$B$, but for the next proof it is useful to do so.
...
We can then find the maximum flow in time $O(n\log n)$ time, using e.g. the
planar max-flow algorithm by Erickson~\cite{E10}.
...
The Hungarian search takes $O(r\sqrt{n}\polylog n)$ time.
\end{lemma}
\begin{proof}

...
$O(r\sqrt{n}(r+\sqrt{n})\polylog n)$ time.
\end{theorem}
...
Computing $(1+\eps)$-approximate geometric partial matching can be reduced to the following problem in $O(n \polylog n)$ time:
...
%
Standard charging argument shows that the amortized update time is $O(r\sqrt{n}(r+\sqrt{n})\polylog n)$; for a complete argument see Appendix~\ref{SSA:update-support-star}.
...
\note{A MESS; NEED TO BE REWRITTEN}
...
bounded above by twice of its length.
%(each vertex may be removed from one star, and/or added to another star).
Thus, each membership change can be performed in $O(\polylog n)$ time, and there are $O(rn\log n)$ many.

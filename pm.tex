...
\section{Approximating Min-cost Partial Matchings}
...
from Goldberg, Kaplan, Hed, and Tarjan~\cite{GHKT17}.
...
Combining
Observation~\ref{observation:reduction_cost}, Corollary~\ref{corollary:goldberg_cost_add}, and
Lemma~\ref{lemma:sa_partition}, one has
\begin{align*}
	\cost(\bigcup_{T \in T_{j_2}} M_{f_T})
		&= \sum_{T \in T_{j_2}} \cost(M_{f_T}) & \\
		&= \sum_{T \in T_{j_2}} \cost(f_T) & \text{\small [Observation~\ref{observation:reduction_cost}]}\\
		&\leq \sum_{T \in T_{j_2}} (\cost(f^*_T) + \eps c(e_{j_1})/n) & \text{\small [Corollary~\ref{corollary:goldberg_cost_add}]} \\
		&= \sum_{T \in T_{j_2}} (\cost(M^*_T) + \eps c(e_{j_1})/n) & \text{\small [Observation~\ref{observation:reduction_cost}]} \\

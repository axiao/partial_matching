...
excess.
There are $|E_{>0}(f)| = O(k)$ reverse arcs possible, so the total created
excess must be $O(k)$.
\end{proof}
...
\item Empty 2-paths from $s$ to vertices of $B_\emptyset \setminus S$, if
	$s \in S$, maintained in BCP data structure $D_\emptyset(A)$.
\item Empty 2-paths from vertices of $A_\emptyset \cap S$ to $t$, if
...
\note{need a formal statement that that $|E_{>0}(f)| = O(k)$}
\end{proof}
...
%TODO removing the mechanics of updates to outside
...
lazily update potentials after vertices leave $S$.
To remind, the number of potential updates outside of empty vertices was
bounded by the number of relaxations performed by the most recent invocation
of Hungarian search, and thus $O(k)$ by Lemma~\ref{lemma:goldberg_hs_length1}
and \ref{lemma:goldberg_hs_length2}.
...
\note{probably move correctness of this choice into its own lemma; ``feasible choice''}
...
relies on the relaxation of a minimum-reduced arc or empty path.
\begin{algorithm}
...
\Function{DFS}{$H = (V, E)$, $f$, $\pi$}
	...
	\Repeat
		...
		\If{$\gamma \leq 0$}
			...
			\State $P \gets$ \Call{Push}{$P$, $v_\ell$}
		\EndIf

		\If{$e_f(v_\ell) < 0$} \Comment{$v_\ell$ is a deficit}
			\State\Return $(f, \pi)$
		\EndIf
	\Until{$S = \emptyset$}
	\State\Return failure

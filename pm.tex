...
\paragraph{Network.}
A \EMPH{network $N(G)$} is a graph $G$ augmented with edge
...
A \EMPH{unit-capacity} network $N(G)$ has all its edge capacityies equal to $1$. \note{Do we every have non-unit-capacity network in the section? Residual ones?}

\paragraph{Pseudoflows.}
A \EMPH{pseudoflow} $f\colon E(G) \to \ints$ is a function on the edges of $G$
satisfying $f(v, w) \leq u(v, w)$ for every edges$(v, w)$.
We say that $f$ \EMPH{saturates} an arc $(v, w)$ if $f(v, w) = u(v, w)$.
The \EMPH{support} of $f$, denoted as \EMPH{$E_{>0}(f)$}, is the set of edges with positive flows:
\[
...
A vertex is \EMPH{balanced} if it has zero imbalance.
If all vertices are balanced, the pseudoflow is a \EMPH{circulation}.
The \EMPH{cost} of a pseudoflow, denoted \EMPH{$\cost(f)$}, is defined to be
\[
 \cost(f) \coloneqq \coloneqq \sum_{(v, w) \in E(G)} c(v, w) \cdot f(v, w).
\]
Given a network $N(G)$,
the \EMPH{minimum-cost flow problem (MCF)} asks to find a circulation of minimum cost inside $N(G)$.

\paragraph{Residual network.}
For each edge $(v, w)$, the \EMPH{residual capacity $u_f$} with respect to
pseudoflow $f$ is defined to be $u_f(v, w) \coloneqq u(v, w) - f(v, w)$.
The set of \EMPH{residual arcs} is defined as
\[
\EMPH{$E(G)_f$} \coloneqq \{(v, w) \in E(G) \mid u_f(v, w) > 0\}.
\]
Define \EMPH{$N(G)_f$} to be the \EMPH{residual newwork} with respect to pseudoflow $f$, consisting of the \EMPH{residual graph $G_f$} on vertex set $V(G)$ and edge set $E(G)_f$, together with the antisymmetric cost function $c$, residual capacities $u_f$, and the supply-demand function $e_f$. \note{Is this how the function should change? We should rename $e_f$ to $\fsupply_f$?}
...
$0 \leq |e_{f'}(v)| \leq |e_f(v)|$ for every vertex $v$,
\item
$e_{f'}(v)$ and $e_f(v)$ share the same sign on every vertex $v$, and
%\item
% if $e_f(v) = 0$ then $e_{f'}(v) = 0$, \note{this follows from (1) or (2)} and
...
$e_f(v) \geq \Delta$, and a deficit vertex with $e_f(v) \leq -\Delta$.
Vertices with $\abs{e_f(v)} \geq \Delta$ are called \EMPH{active}.
...
$\abs{e_f(v)} \geq \alpha\Delta$, for a parameter $\alpha \in (1/2, 1)$.
...
Third, $\Delta$ is aggressively lowered to $\max_{v \in V} e_f(v)$ if there are
...
%Ultimately, we substitute $G$ with a new graph $\tilde{G}$ with only $O(r)$
%vertices (thus, $O(r)$ relaxations), and where each relaxation can be performed
%in $O(\sqrt{n}\polylog n)$ time.

\paragraph{Dead vertices.}
We call a vertex $b \in B$ \EMPH{dead} if it has no adjoining
$E_{>0}(f)$ arcs and is not an active excess or deficit.
...
%TODO i don't think we're really making a substitute graph this time. need to keep individual potentials for \Sigma_a
%\paragraph{Compressed graph.}
%We now construct $\tilde{G}$ from $G$.
%First, remove all dead vertices and their adjacent arcs from $G$.
%Next, for each support star $\Sigma_a$ and its center $a$, we replace 


\subsection{Implementation details}

%Agarwal~\etal~\cite{AFPVX17} provide a geometric implementation of Orlin's
%algorithm to solve transportation problem between point sets $A$, $B$ lying in
%the plane, using $L_p$ distances as costs.
%As a result, they are able to compute each augmenting path in $O(n\polylog n)$
%time and recover the optimal flow in $O(n^2\polylog n)$ time.

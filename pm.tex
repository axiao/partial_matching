...
Given a reduction network $N$ over a point set with diameter at most $K \cdot kn^2$ for some constant $K$, compute an $(K \cdot \eps/6kn)$-optimal circulation on $N$.
\end{lemma}
...
\note{Need the argument that puts the clusters at a distance that does not change the optimal solution, but with bounded diameter.}
...
%The proof of these properties can be found in the original paper \cite[Section 3.5]{SA12}, but we reproduce the rest of proofs below

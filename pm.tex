...
\note{Can we just use Lemma~\ref{lemma:reduction_count}?}
...
Each desaturation of backward arcs creates one unit of excess.  By Lemma~\ref{lemma:reduction_count},
the number of backward arcs is at most $O(k)$.  Thus the total amount of excess in the final $\eps$-optimal flow is $O(k)$.

The whole precedure \textsc{Scale-Init} can be performed in $O(n)$ time.

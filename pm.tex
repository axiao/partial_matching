...
\begin{proof}
The proof of this lemma is adapted from Goldberg~\etal~\cite{GHKT17}.
...
Next, consider cuts $\EMPH{$Y_i$} \coloneqq \{v \mid d(v) > i\}$ for $0 \leq i \leq \sqrt{k}$.
By $\theta$-optimality of $f$ and $2\theta$-optimality of $f_0$,
any $\arc vw \in E^+$ satisfies
\[
d(w) - d(v) = \frac{1}{\theta}\Paren{(\pi(w) - \pi_0(w)) - (\pi(v) - \pi_0(v)}
	= \frac{1}{\theta}\Paren{c_{\pi_0}(\arc wv) + c_\pi(\arc vw)}
	\geq -3.
\]
Rearranging, $d(w) \geq d(v) - 3$, and each arc of $E^+$ crosses at most 3 cuts $Y_i$.
Furthermore, the size of $E^+$ is $O(k)$, bounded by the support size of $f$ and $f_0$.
Averaging, there is a cut among $Y_i$s containing at most $3k/\sqrt{k}$ arcs of $E^+$,
...
We use a standard reduction to the uncapacitated min-cost flow problem.
...
In this section, we use Orlin's algorithm~\cite{O93} as well as some of
...
When the algorithm terminates, an optimal circulation in the contracted network is found.
In Orlin's original algorithm~\cite{O93}, the optimal circulation is then
recovered by solving a max flow problem on the set of admissible arcs under the
optimal potential.

The algorithm in Agarwal~\etal~\cite{AFPVX17arxiv} implements Orlin's algorithm
geometrically such that each augmentation can be done in $O(n\polylog n)$ time.
This includes using BCP to query minimum-reduced cost arcs, and implementing
contractions in a way that preserves individual point identities for use in the BCP.
Finally, they also provide an $O(n\polylog n)$ time post-processing step for
recovering the optimal circulation from optimal potential.

In the next subsection, we describe an algorithm that, after $O(n\polylog n)$ time
preprocessing, finds an admissible excess-to-deficit path in
$O(r\sqrt{n} \polylog n)$ amortized time.
Our algorithm diverges slightly from Agarwal~\etal~\cite{AFPVX17arxiv} here,
as we use a Hungarian search to find the augmenting path rather than computing
an entire shortest path tree.
...
\note{TODO reference the correct lemma/proof} %TODO
\begin{lemma}[\cite{AFPVX17arxiv}]
...
The following lemma, coming from the discussion above, bounds the running time
of the Hungarian search.
